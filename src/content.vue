<template>
  ...
  <n-card hoverable style="display: flex; flex-direction: column; height: 100%;">
    <n-space justify="center" style="flex-grow: 3; width: 100%;" item-style="width: 32%;">
      <!-- 第一个输入框 -->
      <n-input-number v-model:value="capacity" :format="format" :parse="parse">
        <template #prefix>储能单元容量：</template>
        <template #suffix>kW</template>
      </n-input-number>
      <!-- 第二个输入框 -->
      <n-input-number v-model:value="rate" :format="format" :parse="parse">
        <template #prefix>充放电倍率：</template>
        <template #suffix>C </template>
      </n-input-number>
      <!-- 第三个输入框 -->
      <n-input-number v-model:value="eff" :format="format" :parse="parse">
        <template #prefix>储能系统效率：</template>
      </n-input-number>
    </n-space>
  </n-card>
  ...
  <n-grid cols="4" item-responsive>
    ...
    <n-grid-item span="1" style="padding: 0.2rem;">
      <n-card hoverable title="优化调度数据" class="center-title-small" style="height: 500px;">
        ...
        <p style="text-align: left; font-size: 28px;">
          充放电量：{{ totalCharge.toFixed(2) }} kWh
        </p>
      </n-card>
    </n-grid-item>
  </n-grid>
...
const calculateAndUpdateChart = async () => {
  try {
    ...
    if (data.code === 200) {
      ...
      chargeData.value = data.data.chargeData
      dischargeData.value = data.data.dischargeData
      totalRevenue.value = data.data.totalRevenue
      totalCharge.value = data.data.totalCharge
